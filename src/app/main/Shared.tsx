...
export function StateDropdown({selectedState, setSelectedState, includesAll}:StateDropdownProps) {
    ...
    return (
        <Dropdown direction="up" isOpen={isOpen} toggle={toggle}>
            ...
            <DropdownMenu>
                {includesAll ? <DropdownItem key={'All'} onClick={() => setSelectedState('All')}>{'All'}</DropdownItem> : <React.Fragment/> }
                {STATES.map(state => {
